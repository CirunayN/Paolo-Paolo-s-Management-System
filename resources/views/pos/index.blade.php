    ...
    <div class="w-full lg:w-[420px] shrink-0 bg-white dark:bg-[#0c1222] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col overflow-hidden">
        ...
        <div class="p-3.5 border-b border-slate-200 dark:border-slate-800 space-y-2.5 bg-slate-50/50 dark:bg-dark-850/50">
            <div class="flex items-center justify-between">
                ...
                <button type="button" onclick="promptClearCart()" class="text-xs font-semibold text-rose-500 hover:text-rose-600 flex items-center gap-1 transition-colors">
...
function promptClearCart() {
    ...
    const countEl = document.getElementById('clearCartItemCount');
...
<!-- CONFIRMATION MODAL: Clear POS Terminal Cart -->
<div id="clearCartModal" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm hidden items-center justify-center p-4">
    <div class="w-full max-w-sm bg-white dark:bg-[#0c1222] border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150">
        <div class="flex items-center gap-3.5 text-rose-500">
            <div class="w-11 h-11 rounded-2xl bg-rose-500/10 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400 flex items-center justify-center text-lg shrink-0 border border-rose-500/20">
                <i class="fas fa-trash-can"></i>
            </div>
            <div>
                <h3 class="font-bold text-slate-900 dark:text-white text-base font-display">Clear Terminal Cart?</h3>
                <p class="text-xs text-slate-500 dark:text-slate-400">Avoid accidental order cancellation</p>
            </div>
        </div>

        <p class="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
            Are you sure you want to remove all items (<span id="clearCartItemCount" class="font-bold text-rose-500">0 items</span>) from this checkout order?
        </p>

        <div class="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-200 dark:border-slate-800">
            <button type="button" onclick="closeClearCartModal()" class="px-4 py-2 rounded-xl bg-slate-100 dark:bg-dark-800 hover:bg-slate-200 dark:hover:bg-dark-700 text-slate-700 dark:text-slate-300 text-xs font-bold transition-colors">
                Cancel
            </button>
            <button type="button" onclick="confirmClearCart()" class="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold shadow-lg shadow-rose-600/20 transition-all flex items-center gap-1.5">
