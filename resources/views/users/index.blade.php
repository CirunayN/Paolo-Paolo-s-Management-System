    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold font-display text-slate-900 dark:text-white flex items-center gap-2.5">
                <i class="fas fa-user-shield text-cyan-600 dark:text-cyan-400"></i> Staff &amp; User Management
            </h2>
        </div>
            ...
            <table class="w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-800/60" data-auto-animate>
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-dark-800/40 transition-colors">
                        ...
                        <td class="py-3.5 px-4 font-mono text-cyan-600 dark:text-cyan-400 font-semibold">
                            {{ $u->username ?: '-' }}
                        </td>
                        ...
                        <td class="py-3.5 px-4">
                            <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase {{ $u->role === 'admin' ? 'bg-purple-500/15 text-purple-700 dark:text-purple-300 border border-purple-500/30' : 'bg-blue-500/15 text-blue-700 dark:text-blue-300 border border-blue-500/30' }}">
                                {{ $u->role }}
                            </span>
                        </td>
                        ...
                        <td class="py-3.5 px-4 text-center">
                            <span class="px-2 py-0.5 rounded text-[10px] font-bold {{ $u->is_active ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300' : 'bg-rose-500/15 text-rose-700 dark:text-rose-300' }}">
                                {{ $u->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="py-3.5 px-4 text-right space-x-1">
                            <button type="button" onclick="editUser({{ json_encode($u) }})" class="p-1.5 rounded-lg bg-slate-100 dark:bg-dark-800 hover:bg-slate-200 dark:hover:bg-dark-700 text-slate-700 dark:text-slate-300 hover:text-cyan-600 dark:hover:text-cyan-400 text-xs transition-colors border border-slate-300 dark:border-slate-700">
                            ...
                            <form method="POST" action="{{ route('users.destroy', $u->id) }}" class="inline" onsubmit="return confirm('Delete account for {{ addslashes($u->name) }}?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-1.5 rounded-lg bg-slate-100 dark:bg-dark-800 hover:bg-rose-50 dark:hover:bg-rose-950/40 text-slate-500 hover:text-rose-600 dark:hover:text-rose-400 text-xs transition-colors border border-slate-300 dark:border-slate-700">
...
<div id="userModal" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm hidden items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-[#0c1222] border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-2xl space-y-4">
        <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
            <h3 id="userModalTitle" class="text-base font-bold font-display text-slate-900 dark:text-white">Create Staff Account</h3>
            <button type="button" onclick="closeUserModal()" class="text-slate-400 hover:text-slate-600 dark:hover:text-white text-xl leading-none">&times;</button>
        </div>
        ...
        <form id="userForm" method="POST" action="{{ route('users.store') }}">
            ...
            <div class="space-y-3 text-xs">
                <div>
                    <label class="block text-slate-700 dark:text-slate-300 font-semibold mb-1">Full Name <span class="text-rose-500">*</span></label>
                ...
                <div class="grid grid-cols-2 gap-2.5">
                    <div>
                        <label class="block text-slate-700 dark:text-slate-300 font-semibold mb-1">Username <span class="text-rose-500">*</span></label>
                        <input type="text" name="username" id="uUsername" required placeholder="e.g. cashier2"
                            class="w-full py-2 px-3 bg-slate-50 dark:bg-dark-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white text-xs focus:ring-1 focus:ring-cyan-500">
                    </div>
                    <div>
                        <label class="block text-slate-700 dark:text-slate-300 font-semibold mb-1">Role <span class="text-rose-500">*</span></label>
                        <select name="role" id="uRole" required class="w-full py-2 px-3 bg-slate-50 dark:bg-dark-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white text-xs focus:ring-1 focus:ring-cyan-500">
                ...
                <div>
                    <label class="block text-slate-700 dark:text-slate-300 font-semibold mb-1">Email Address <span class="text-rose-500">*</span></label>
                ...
                <div>
                    <label class="block text-slate-700 dark:text-slate-300 font-semibold mb-1">
                        Password <span id="pwdNotice" class="text-slate-400 text-[10px] font-normal">(Leave blank to keep current)</span>
                    </label>
                ...
                <div id="activeToggleContainer" class="flex items-center pt-1 hidden">
                    <label class="flex items-center text-xs text-slate-700 dark:text-slate-300 cursor-pointer">
                        <input type="checkbox" name="is_active" id="uIsActive" value="1" class="w-4 h-4 rounded bg-slate-100 dark:bg-dark-900 border-slate-300 dark:border-slate-700 text-cyan-500 mr-2">
            ...
            <div class="flex items-center justify-end gap-3 pt-4 mt-4 border-t border-slate-200 dark:border-slate-800">
                <button type="button" onclick="closeUserModal()" class="px-4 py-2 rounded-xl bg-slate-100 dark:bg-dark-800 hover:bg-slate-200 dark:hover:bg-dark-700 text-slate-700 dark:text-slate-300 font-semibold text-xs">
                    Cancel
                </button>
