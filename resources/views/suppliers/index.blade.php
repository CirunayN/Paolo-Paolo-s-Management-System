    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold font-display text-white flex items-center gap-2.5">
                <i class="fas fa-truck-moving text-cyan-400"></i> Matting &amp; Accessories Suppliers
            </h2>
            <p class="text-xs text-slate-400">Direct manufacturers, importers, and distributor contacts</p>
        </div>
        ...
        <form method="GET" action="{{ route('suppliers.index') }}" class="flex gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search supplier name, contact person, or phone number..."
                    class="w-full pl-9 pr-4 py-2 bg-dark-900 border border-slate-700/80 rounded-xl text-white text-xs placeholder-slate-500 focus:ring-1 focus:ring-cyan-500">
            </div>
                ...
                <tbody class="divide-y divide-slate-800/60" data-auto-animate>
                    ...
                    <tr class="hover:bg-dark-800/40 transition-colors">
                        ...
                        <td class="py-3.5 px-4">
                            <div class="font-mono text-cyan-400 font-semibold">{{ $s->contact_number ?: '-' }}</div>
                            @if($s->email)
                        ...
                        <td class="py-3.5 px-4 text-right space-x-1">
                            <button type="button" onclick="editSupplier({{ json_encode($s) }})" class="p-1.5 rounded-lg bg-dark-800 hover:bg-dark-700 text-slate-300 hover:text-cyan-400 transition-colors">
                            ...
                            <form method="POST" action="{{ route('suppliers.destroy', $s->id) }}" class="inline" onsubmit="return confirm('Delete supplier {{ addslashes($s->supplier_name) }}?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-1.5 rounded-lg bg-dark-800 hover:bg-rose-950/40 text-slate-400 hover:text-rose-400 transition-colors">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="py-8 text-center text-slate-500">No suppliers found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="supplierModal" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm hidden items-center justify-center p-4">
    <div class="w-full max-w-md bg-[#0c1222] border border-slate-700 rounded-2xl p-6 shadow-2xl space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 id="modalTitle" class="text-base font-bold font-display text-white">Add Supplier</h3>
            <button type="button" onclick="closeSupplierModal()" class="text-slate-400 hover:text-white">&times;</button>
        </div>
        ...
        <form id="supplierForm" method="POST" action="{{ route('suppliers.store') }}">
            ...
            <div class="space-y-3 text-xs">
                <div>
                    <label class="block text-slate-300 font-semibold mb-1">Company / Supplier Name <span class="text-rose-400">*</span></label>
                ...
                <div>
                    <label class="block text-slate-300 font-semibold mb-1">Warehouse / Office Address</label>
                    <input type="text" name="address" id="supAddress" placeholder="City, Metro Manila / Province"
                        class="w-full py-2 px-3 bg-dark-900 border border-slate-700 rounded-xl text-white text-xs focus:ring-1 focus:ring-cyan-500">
                </div>

                <div>
                    <label class="block text-slate-300 font-semibold mb-1">Notes / Supplied Products</label>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 mt-4 border-t border-slate-800">
                <button type="button" onclick="closeSupplierModal()" class="px-4 py-2 rounded-xl bg-dark-800 text-slate-300 font-semibold text-xs">
                    Cancel
                </button>
